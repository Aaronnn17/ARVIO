...
export function canDirectPlayMkvStream(stream: CompatStream): boolean {
  ...
  return !!version && Number(version[1]) >= 145 && /^(mkv|matroska)$/.test(streamContainer(stream)) && !videoReason(stream) && !audioReason(stream);
}
...
export function playbackPlan(stream: CompatStream): PlaybackPlan {
  ...
  return {
    ...
    detail: mode === "transcode" ? "Server conversion requires provider support and permission" : reason
  };
}
