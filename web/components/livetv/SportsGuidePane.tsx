...
export function SportsGuidePane({ channels, guide, onPlay, onEnter, onOpenCategories, providerNames = {}, addons = NO_ADDONS }: {
  ...
  useEffect(() => {
    let active = true;
    setArtwork([]);
    void loadSportsGuideArtwork(addons).then(items => { if (active) setArtwork(items); });
    return () => { active = false; };
  }, [addons]);
  ...
  useEffect(() => {
    ...
    worker.onmessage = (event: MessageEvent<SportsGuideEvent[]>) => { setEvents(event.data); setLoading(false); };
    worker.onerror = () => { setEvents([]); setLoading(false); };
    worker.postMessage({ channels: channels.filter((ch) => Boolean(guide[ch.id])), guide, now });
