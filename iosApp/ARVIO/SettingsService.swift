...
struct CloudProfileSettings: Codable, Equatable {
    ...
    var subtitleOffset: String = "Bottom"
    ...
    var trailerDelaySeconds: Int = 2
    var clockFormat: String = "24h"
    var showBudget: Bool = true
    var showLoadingStats: Bool = true
    var smoothScrolling: Bool = true
    var oledBlackBackground: Bool = false
    ...
    init(
        ...
        trailerDelaySeconds: Int = 2,
        clockFormat: String = "24h",
        showBudget: Bool = true,
        showLoadingStats: Bool = true,
        smoothScrolling: Bool = true,
        oledBlackBackground: Bool = false,
        ...
        self.trailerDelaySeconds = trailerDelaySeconds
        self.clockFormat = clockFormat
        self.showBudget = showBudget
        self.showLoadingStats = showLoadingStats
        self.smoothScrolling = smoothScrolling
        self.oledBlackBackground = oledBlackBackground
    ...
    init(from decoder: Decoder) throws {
        ...
        subtitleOffset = (try? container.decode(String.self, forKey: .subtitleOffset)) ?? "Bottom"
        ...
        trailerDelaySeconds = (try? container.decode(Int.self, forKey: .trailerDelaySeconds)) ?? 2
        ...
        showLoadingStats = (try? container.decode(Bool.self, forKey: .showLoadingStats)) ?? true
        smoothScrolling = (try? container.decode(Bool.self, forKey: .smoothScrolling)) ?? true
        oledBlackBackground = (try? container.decode(Bool.self, forKey: .oledBlackBackground)) ?? false
        spoilerBlurEnabled = (try? container.decode(Bool.self, forKey: .spoilerBlurEnabled)) ?? false
        accentColor = (try? container.decode(String.self, forKey: .accentColor)) ?? "White"
...
final class SettingsService: ObservableObject {
    ...
    func loadFromCloud() {
        if let settings = cloud.payload.profileSettingsById?[activeProfileId] {
            ...
            profileSettings = CloudProfileSettings(
                ...
                trailerAutoPlay: cloud.payload.trailerAutoPlay ?? false,
                trailerSoundEnabled: cloud.payload.trailerSoundEnabled ?? false,
                trailerDelaySeconds: cloud.payload.trailerDelaySeconds ?? 2,
                clockFormat: cloud.payload.clockFormat ?? "24h",
                showBudget: cloud.payload.showBudget ?? true,
                showLoadingStats: cloud.payload.showLoadingStats ?? true,
                smoothScrolling: cloud.payload.smoothScrolling ?? true,
                oledBlackBackground: cloud.payload.oledBlackBackground ?? false,
                spoilerBlurEnabled: cloud.payload.spoilerBlurEnabled ?? false,
                accentColor: cloud.payload.accentColor ?? cloud.payload.focusBorderColor ?? "White",
                volumeBoostDb: cloud.payload.volumeBoostDb ?? 0,
                includeSpecials: cloud.payload.includeSpecials ?? false,
                dnsProvider: cloud.payload.dnsProvider ?? "system",
                customUserAgent: cloud.payload.customUserAgent ?? "",
