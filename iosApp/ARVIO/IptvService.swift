...
struct IptvChannel: Identifiable, Codable, Hashable {
    ...
    let epgId: String?
    let rawTitle: String
    let xtreamStreamId: Int?
    let catchupDays: Int
    let catchupType: String?
    ...
    init(from decoder: Decoder) throws {
        ...
        rawTitle = (try? container.decode(String.self, forKey: .rawTitle)) ?? name
        xtreamStreamId = try? container.decode(Int.self, forKey: .xtreamStreamId)
        catchupDays = (try? container.decode(Int.self, forKey: .catchupDays)) ?? 0
        catchupType = try? container.decode(String.self, forKey: .catchupType)
        catchupSource = try? container.decode(String.self, forKey: .catchupSource)
    }
}
...
final class IptvService: ObservableObject {
    ...
    func savePlaylists(_ playlists: [IptvPlaylistEntry]) async {
        state.playlists = playlists
            .map { playlist in
                IptvPlaylistEntry(
                    id: playlist.id.nilIfBlank ?? UUID().uuidString,
                    name: playlist.name.nilIfBlank ?? "Playlist",
                    m3uUrl: playlist.m3uUrl.trimmingCharacters(in: .whitespacesAndNewlines),
                    epgUrl: playlist.epgUrl.trimmingCharacters(in: .whitespacesAndNewlines),
                    enabled: playlist.enabled
                )
            }
            .filter { !$0.m3uUrl.isEmpty }
        state.m3uUrl = state.playlists.first?.m3uUrl ?? ""
        state.epgUrl = state.playlists.first?.epgUrl ?? ""
        saveLocal()
        await cloud.save(iptv: state, profileId: activeProfileId)
        await reload()
    }

    func addPlaylist(name: String, m3uUrl: String, epgUrl: String) async {
        let m3u = m3uUrl.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !m3u.isEmpty else { return }
        var playlists = state.playlists
        if playlists.isEmpty, !state.m3uUrl.isEmpty {
            playlists.append(IptvPlaylistEntry(
                id: "list_1",
                name: "List 1",
                m3uUrl: state.m3uUrl,
                epgUrl: state.epgUrl,
        ...
        playlists.append(IptvPlaylistEntry(
            id: "list_\(UUID().uuidString)",
            name: name.nilIfBlank ?? "Playlist \(playlists.count + 1)",
            m3uUrl: m3u,
            epgUrl: epgUrl.trimmingCharacters(in: .whitespacesAndNewlines),
            enabled: true
        ))
        await savePlaylists(playlists)
    }

    func togglePlaylist(_ playlist: IptvPlaylistEntry) async {
        var playlists = normalizedEditablePlaylists()
        guard let index = playlists.firstIndex(where: { $0.id == playlist.id }) else { return }
        playlists[index].enabled.toggle()
        await savePlaylists(playlists)
    }

    func movePlaylist(_ playlist: IptvPlaylistEntry, direction: Int) async {
        var playlists = normalizedEditablePlaylists()
        guard let index = playlists.firstIndex(where: { $0.id == playlist.id }) else { return }
        let nextIndex = max(0, min(playlists.count - 1, index + direction))
        guard nextIndex != index else { return }
        playlists.swapAt(index, nextIndex)
        await savePlaylists(playlists)
    }

    func removePlaylist(_ playlist: IptvPlaylistEntry) async {
        var playlists = normalizedEditablePlaylists()
    ...
    func editablePlaylists() -> [IptvPlaylistEntry] {
        normalizedEditablePlaylists()
    }
    ...
    private func normalizedEditablePlaylists() -> [IptvPlaylistEntry] {
        if !state.playlists.isEmpty { return state.playlists }
        guard !state.m3uUrl.isEmpty else { return [] }
        return [IptvPlaylistEntry(id: "list_1", name: "List 1", m3uUrl: state.m3uUrl, epgUrl: state.epgUrl, enabled: true)]
    }
    ...
    private func fetchXtreamLiveChannels(credentials: XtreamCredentials) async throws -> [IptvChannel] {
        ...
        return streams.compactMap { stream -> IptvChannel? in
            ...
            return IptvChannel(
                ...
                catchupDays: max(stream.tvArchiveDuration ?? 0, stream.tvArchive ?? 0),
                catchupType: (max(stream.tvArchiveDuration ?? 0, stream.tvArchive ?? 0) > 0) ? "xtream" : nil
            )
        }
    }
    ...
    private func parseM3U(_ text: String) -> [IptvChannel] {
        ...
        for rawLine in text.components(separatedBy: .newlines) {
            ...
            if line.hasPrefix("#EXTINF") {
                ...
                let catchupDays = Int(
                    SharedCoreBridge.m3uAttribute(line: line, key: "catchup-days") ??
                    SharedCoreBridge.m3uAttribute(line: line, key: "timeshift") ??
                    ""
                ) ?? 0
                let catchupType = SharedCoreBridge.m3uAttribute(line: line, key: "catchup")?.nilIfBlank
                let catchupSource = SharedCoreBridge.m3uAttribute(line: line, key: "catchup-source")?.nilIfBlank
                pending = (name, group, logo, epgId, line, catchupDays, catchupType, catchupSource)
            } else if line.hasPrefix("http") {
                let meta = pending ?? ("Channel \(result.count + 1)", "Uncategorized", nil, nil, "", 0, nil, nil)
                result.append(
                    IptvChannel(
...
private struct XtreamLiveStream: Decodable {
    ...
    init(from decoder: Decoder) throws {
        ...
        tvArchive = try? container.decodeLossyInt(forKey: .tvArchive)
        tvArchiveDuration = try? container.decodeLossyInt(forKey: .tvArchiveDuration)
...
private extension KeyedDecodingContainer {
    func decodeLossyInt(forKey key: Key) throws -> Int {
        ...
        throw DecodingError.valueNotFound(
            Int.self,
            DecodingError.Context(codingPath: codingPath + [key], debugDescription: "No int-compatible value")
        )
    }
